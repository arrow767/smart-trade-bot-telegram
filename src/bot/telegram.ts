...
// единая безопасная обёртка — чтобы бот никогда не падал
async function safe<T>(ctx: any, fn: () => Promise<T>) {
  try {
    return await fn();
  } catch (e: any) {
    const msg = e?.description || e?.message || String(e);
    try { await ctx.reply(`❌ <b>ERROR</b>: ${escapeHtml(msg)}`, { parse_mode: "HTML" }); } catch {}
  }
}

// доступ + удобные хелперы
function isAllowedCtx(ctx: any): boolean {
  const chatId = Number(ctx.chat?.id ?? ctx.from?.id);
  const user = ctx.from?.username ? String(ctx.from.username) : undefined;
  return whitelist.isAllowed(chatId, user);
}
function deny(ctx: any) {
  const chatId = Number(ctx.chat?.id ?? ctx.from?.id);
  const user = ctx.from?.username;
  return ctx.reply(Whitelist.formatDenied(chatId, user), { parse_mode: "HTML" });
}
...
// Полный help-текст (как в консоли «9»)
...
// /start — автопривязка первого пользователя если whitelist пуст и .env не задан
bot.start((ctx) => safe(ctx, async () => {
  const chatId = Number(ctx.chat?.id ?? ctx.from?.id);
  const username = ctx.from?.username;

  if (!whitelist.lockedByEnv) {
    // если allowed.json пуст — привязываем первого
    whitelist.bindFirstIfEmpty(chatId, username);
  }

  if (!isAllowedCtx(ctx)) return deny(ctx);
...
// HELP — всегда отправляем НОВОЕ сообщение (reply), не editMessageText
bot.action("HELP", (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
...
bot.action(/CLOSE\|([a-zA-Z0-9]+)\|([0-9]{1,3})/, (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  await ctx.answerCbQuery();
  const symbol = ctx.match![1];
  const pct = Math.max(1, Math.min(100, Number(ctx.match![2])));
  await runCommand(ex, book, { kind:"close", symbol, percent: pct },
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    "telegram");
}));

bot.action("DEP", (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  await ctx.answerCbQuery();
  await runCommand(ex, book, {kind:"deposit"},
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    "telegram");
}));

bot.action("TASKS", (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  await ctx.answerCbQuery();
  ...
  for (const t of rows) {
    ...
    await ctx.reply(
      `<b>#${t.id}</b> [${t.status}] ${escapeHtml(t.symbolCcxt)}\n${escapeHtml(t.label)}\n(+${ago}s)${t.error?`\nERR: ${escapeHtml(t.error)}`:""}`,
      { parse_mode:"HTML", ...kb }
    );
  }
...
bot.action(/CANCEL\|([0-9]+)/, (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  await ctx.answerCbQuery(`Cancel #${ctx.match![1]}`);
  const id = Number(ctx.match![1]);
  await runCommand(ex, book, { kind:"cancel", id },
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    "telegram");
}));

bot.action("CANCEL_ALL", (ctx) => safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  await ctx.answerCbQuery("Cancel all");
  await runCommand(ex, book, { kind:"cancel_all" },
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    (m)=>ctx.reply(m,{parse_mode:"HTML"}),
    "telegram");
}));

// /whoami — чтобы быстро узнать chatId/username для .env
bot.command("whoami", (ctx)=> safe(ctx, async () => {
  const chatId = Number(ctx.chat?.id ?? ctx.from?.id);
...
bot.on("text", (ctx)=> safe(ctx, async () => {
  if (!isAllowedCtx(ctx)) return deny(ctx);
  ...
  if (/^[0-9]$/.test(text)) {
    ...
    if (cmd) return runCommand(ex, book, cmd,
      (m)=>ctx.reply(m, { parse_mode:"HTML" }),
      (m)=>ctx.reply(m, { parse_mode:"HTML" }),
      "telegram");
  }
  ...
  await runCommand(ex, book, parsed,
    (m)=>ctx.reply(m, { parse_mode:"HTML" }),
    (m)=>ctx.reply(m, { parse_mode:"HTML" }),
    "telegram");
}));
