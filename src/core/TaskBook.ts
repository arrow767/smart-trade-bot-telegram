  ...
  async cleanupOrphanTasks(
    ...
    try {
      ...
      for (const task of tasks) {
        ...
        // ✅ НОВОЕ: Для задач с ошибками - проверяем что нет ни сделок, ни отложек
        if (task.status === "error") {
          // Проверяем что нет входных ордеров
          if (!hasEntryOrders && flat) {
            log(`🧹 Задача с ошибкой #${task.id} (${symbolCcxt}): отложек нет, позиции нет → удаляю`);
            this.remove(task.id);
          }
          continue;
        }
