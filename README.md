...
## 🎯 Методология SL/TP (ключевая логика)

### Основные принципы

1. **SL выставляется СРАЗУ** после первого fill (исполнения отложки)
2. **TP выставляются ТОЛЬКО когда ВСЕ отложки задачи исполнены** (`allLegsFilled = true`)
3. **Расчёт SL/TP от средней цены ЗАДАЧИ**, не от средней позиции с биржи
4. **TP = кратное расстояние до SL** (из пресета: 1:3, 1:5, 1:7 и т.д.)

### Пример: задача с 2 отложками

```
Задача: long SOL, risk=$5, 2 отложки по $100 каждая (totalUsd=$200)
SL расстояние = risk / totalUsd = $5 / $200 = 2.5%
```

**Шаг 1: Срабатывает первая отложка ($100)**
- SL выставляется на 2.5% от средней задачи
- При убытке ~$2.5 (на $100 позиции)
- TP НЕ выставляем — ждём вторую отложку

**Шаг 2: Срабатывает вторая отложка ($100)**
- SL корректируется от новой средней задачи
- При убытке = $5 (на $200 позиции)
- **TP выставляем** на весь объём позиции:
  - TP1 (1:3) = средняя + 7.5% (3 × 2.5%)
  - TP2 (1:5) = средняя + 12.5% (5 × 2.5%)
  - TP3 (1:7) = средняя + 17.5% (7 × 2.5%)

### Цепочка задач (добор к существующей позиции)

```
Позиция: $200 SOL (Task #1)
Новая задача: Task #2, risk=$5, 2 отложки по $100
```

**При первом fill Task #2:**
1. Task #1 помечается `supersededBy = Task #2`
2. TP старой задачи снимаются
3. SL пересчитывается от средней Task #2 (на 2.5%)
4. TP НЕ ставим — ждём все отложки Task #2

**После всех fill Task #2:**
1. TP выставляются на **весь объём** ($400) 
2. Цены TP от **средней Task #2**
3. Расстояние = risk Task #2 / totalUsd Task #2

### Отслеживание TP (filledTpCount)

- При исполнении TP → `filledTpCount++`
- При восстановлении TP → пропускаем первые N (где N = filledTpCount)
- Гарантирует что на место съеденного TP не ставится новый

### Recovery (рестарт бота)

При рестарте бота:
1. **Автоопределение allLegsFilled**: если позиция есть и entry ордеров нет → `allLegsFilled = true`
2. **Проверка SL**: если неправильный → переставляем
3. **Проверка TP**: только если `allLegsFilled = true` и цены неправильные → переставляем

### Поля Task для отслеживания

```typescript
Task {
  entryLegsCount: number;    // Сколько отложек в задаче
  filledLegsCount: number;   // Сколько уже исполнено
  allLegsFilled: boolean;    // Все ли отложки исполнены (TP можно ставить)
  filledTpCount: number;     // Сколько TP съели (не ставить повторно)
  taskEntryAvg: number;      // Средняя цена входа ЗАДАЧИ
  taskEntryQty: number;      // Объём входа ЗАДАЧИ
}
```
